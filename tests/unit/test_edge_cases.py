...
def test_intron_at_chromosome_start():
    ...
    intron = Intron(
        ...
        sequences=IntronSequences(seq="GT" + "N"*96 + "AG", five_prime_dnt="GT", three_prime_dnt="AG"),
        scores=IntronScores(),
...
def test_just_below_minimum_length():
    ...
    intron = Intron(
        intron_id="too_short",
        coordinates=GenomicCoordinate("chr1", 1000, 1029, '+', '1-based'),
        sequences=IntronSequences(seq=seq, five_prime_dnt="GT", three_prime_dnt="AG"),
        scores=IntronScores(),
        metadata=IntronMetadata("t1", "g1")
    )
    ...
    filter_obj = IntronFilter(min_length=30)
    filter_obj._check_omission(intron)
    assert intron.metadata.omitted == 's'  # Short
...
def test_all_noncanonical_types():
    ...
    for five_dnt, three_dnt in nc_types:
        ...
        intron = Intron(
            ...
            sequences=IntronSequences(seq=seq, five_prime_dnt=five_dnt, three_prime_dnt=three_dnt),
            scores=IntronScores(),
...
def test_z_score_normalization_with_zero_variance():
    ...
    # All identical raw scores
    ref_scores = np.array([5.0, 5.0, 5.0, 5.0, 5.0])

    normalizer = ScoreNormalizer()

    # Fit should handle zero variance gracefully
    normalizer.fit(ref_scores)

    # Transform should handle without division by zero
    # Likely returns zeros or raises informative error
    try:
        z_scores = normalizer.transform(ref_scores)
        # If it succeeds, all z-scores should be zero or constant
        assert len(np.unique(z_scores)) == 1
    except ValueError as e:
        # If it fails, should have clear message about variance
        assert "variance" in str(e).lower() or "standard" in str(e).lower()


def test_z_score_with_single_reference():
    """Test normalization with only one reference intron."""
    ref_scores = np.array([5.0])

    normalizer = ScoreNormalizer()
    normalizer.fit(ref_scores)

    # Should handle gracefully
    # Either skip normalization or use default scaling
...
def test_filter_with_all_options_disabled():
    ...
    intron = Intron(
        ...
        sequences=IntronSequences(seq="GTNNNNNNAG", five_prime_dnt="GT", three_prime_dnt="AG"),
        scores=IntronScores(),
        metadata=IntronMetadata("t1", "g1", longest_isoform=False)
    )
...
def test_very_high_scores():
    ...
    intron = Intron(
        ...
        sequences=IntronSequences(seq="GTAG"),
        scores=IntronScores(
            five_raw_score=1e100,
            bp_raw_score=1e100,
            three_raw_score=1e100
        ),
        metadata=IntronMetadata("t1", "g1")
    )
