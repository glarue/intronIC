...
class TestAnnotationParserIntegration:
    ...
    @pytest.mark.skipif(not CHR19_ANNOTATION.exists(), reason="Chr19 annotation not available")
...
class TestParserWriterPipeline:
    ...
    def test_sequence_roundtrip(self, tmp_path):
        ...
        # Format: name  upstream_flank  sequence  downstream_flank  [score]
        seq_file = tmp_path / "test.iic"
        seq_file.write_text(
            "intron1\tAGGCT\tGTAAGTNNNNNTTTAG\tCATGG\t95.5\n"
            "intron2\tTTGCA\tGTAAGANNNNNTTTAG\tGGTAC\t12.3\n"
        )
    ...
    def test_multiple_format_output(self, tmp_path):
        ...
        with SequenceWriter(seq_file) as writer:
            writer.write_intron(intron, species_name="test_species", include_score=False)
        ...
        expected_name = "test_species_TRANS001_3"
        assert bed_name.startswith(expected_name)
...
class TestRealDataPipeline:
    ...
    def test_parse_chr19_and_write_outputs(self, tmp_path):
        ...
        exons = [f for f in features if f.feat_type == "exon"][:10]  # Use first 10 exons
