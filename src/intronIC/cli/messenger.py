...
class UnifiedMessenger:
    ...
    def _get_timestamp(self, force: bool = False) -> str:
        ...
        if force or self.last_timestamp is None or (now - self.last_timestamp).total_seconds() >= 1.0:
    ...
    def error(self, message: str, with_timestamp: bool = False):
        ...
        formatted = f"{timestamp}[bold {PALETTE.error}]✗ {message}[/bold {PALETTE.error}]"
    ...
    def step(
        ...
        if not self.quiet:
            # Section divider
            self.console.print()
            self.console.rule(f"[{PALETTE.header}]Step {step_num}: {step_name}", style=PALETTE.header)

            # Optional: Show pipeline tree with current step highlighted
            if show_tree:
                from rich.tree import Tree
                tree = Tree(f"🔄 [{PALETTE.highlight}]Pipeline Steps", guide_style="dim")
        ...
        self.log_console.print(f"[bold {PALETTE.header}]{step_text}[/bold {PALETTE.header}]")
        self.log_console.print(f"[{PALETTE.header}]{sep_line}[/{PALETTE.header}]")
        self.log_console.print(f"{timestamp}[{PALETTE.info}]Starting step {step_num} of {len(all_steps)}[/{PALETTE.info}]")
    ...
    def print_startup_banner(
        ...
        sep_line = "=" * 80
        self.log_console.print(f"[{PALETTE.header}]{sep_line}[/{PALETTE.header}]")
        self.log_console.print(f"[bold {PALETTE.highlight}]intronIC v{ver}[/bold {PALETTE.highlight}]")
        self.log_console.print(f"[{PALETTE.timestamp}]Started: {timestamp}[/{PALETTE.timestamp}]")
        self.log_console.print(f"[{PALETTE.header}]{sep_line}[/{PALETTE.header}]")
        self.log_console.print("")

        # Combined Command and Configuration section
        self.log_console.print(f"[bold {PALETTE.header}]Command and Configuration:[/bold {PALETTE.header}]")

        # Command and working directory
        if command:
            self.log_console.print(f"  [{PALETTE.table_header}]Command:[/{PALETTE.table_header}] {command}")
        if working_dir:
            self.log_console.print(f"  [{PALETTE.table_header}]Working directory:[/{PALETTE.table_header}] [{PALETTE.path}]{working_dir}[/{PALETTE.path}]")

        # Configuration details
        self.log_console.print(f"  [{ PALETTE.table_header}]Run name:[/{PALETTE.table_header}] {species_name}")
        self.log_console.print(f"  [{PALETTE.table_header}]Input mode:[/{PALETTE.table_header}] {input_mode}")
        self.log_console.print(f"  [{PALETTE.table_header}]Classification threshold:[/{PALETTE.table_header}] [{PALETTE.highlight}]{threshold}%[/{PALETTE.highlight}]")
        self.log_console.print(f"  [{PALETTE.table_header}]Output directory:[/{PALETTE.table_header}] [{PALETTE.path}]{output_dir}[/{PALETTE.path}]")

        # Log input files with full paths
        if genome_path:
            self.log_console.print(f"  [{PALETTE.table_header}]Genome:[/{PALETTE.table_header}] [{PALETTE.path}]{genome_path}[/{PALETTE.path}]")
        if annotation_path:
            self.log_console.print(f"  [{PALETTE.table_header}]Annotation:[/{PALETTE.table_header}] [{PALETTE.path}]{annotation_path}[/{PALETTE.path}]")
        if bed_path:
            self.log_console.print(f"  [{PALETTE.table_header}]BED file:[/{PALETTE.table_header}] [{PALETTE.path}]{bed_path}[/{PALETTE.path}]")
        if sequences_path:
            self.log_console.print(f"  [{PALETTE.table_header}]Sequences:[/{PALETTE.table_header}] [{PALETTE.path}]{sequences_path}[/{PALETTE.path}]")
        if model_path:
            self.log_console.print(f"  [{PALETTE.table_header}]Model:[/{PALETTE.table_header}] [{PALETTE.path}]{model_path}[/{PALETTE.path}]")
    ...
    def print_filtering_summary(
        ...
        total_omitted = omitted_short + omitted_ambiguous + omitted_noncanonical + omitted_isoform + omitted_overlap
        ...
        if not self.quiet:
            ...
            table.add_column("Category", style=PALETTE.table_header, no_wrap=False, width=30)
            table.add_column("Count", style=PALETTE.table_value, justify="right", width=12)
            table.add_column("Percent", style=PALETTE.table_value, justify="right", width=10)
        ...
        self.log_console.print("")
        self.log_console.print(f"[bold {PALETTE.highlight}]Intron Filtering Summary:[/bold {PALETTE.highlight}]")
        self.log_console.print(f"[{PALETTE.table_header}]┌──────────────────────────────┬────────────┬───────────┐[/{PALETTE.table_header}]")
        self.log_console.print(f"[{PALETTE.table_header}]│ Category                     │ Count      │ Percent   │[/{PALETTE.table_header}]")
        self.log_console.print(f"[{PALETTE.table_header}]├──────────────────────────────┼────────────┼───────────┤[/{PALETTE.table_header}]")

        # Show omitted section if any were omitted
        if total_omitted > 0:
            self.log_console.print(f"│ Omitted                      │ {total_omitted:>10,} │ {(total_omitted/total*100) if total > 0 else 0:>8.2f}% │")
            # Show breakdown items that have non-zero counts
            if omitted_short > 0:
                self.log_console.print(f"│   - Too short                │ {omitted_short:>10,} │ {(omitted_short/total*100) if total > 0 else 0:>8.2f}% │")
            if omitted_ambiguous > 0:
                self.log_console.print(f"│   - Ambiguous bases          │ {omitted_ambiguous:>10,} │ {(omitted_ambiguous/total*100) if total > 0 else 0:>8.2f}% │")
            if omitted_noncanonical > 0:
                self.log_console.print(f"│   - Non-canonical            │ {omitted_noncanonical:>10,} │ {(omitted_noncanonical/total*100) if total > 0 else 0:>8.2f}% │")
            if omitted_isoform > 0:
                self.log_console.print(f"│   - Non-longest isoform      │ {omitted_isoform:>10,} │ {(omitted_isoform/total*100) if total > 0 else 0:>8.2f}% │")
            if omitted_overlap > 0:
                self.log_console.print(f"│   - Overlapping              │ {omitted_overlap:>10,} │ {(omitted_overlap/total*100) if total > 0 else 0:>8.2f}% │")

        # Show duplicates row if there are duplicates that weren't also omitted
        if duplicates_only > 0:
            self.log_console.print(f"│ Duplicates only              │ {duplicates_only:>10,} │ {(duplicates_only/total*100) if total > 0 else 0:>8.2f}% │")

        self.log_console.print(f"[{PALETTE.table_header}]├──────────────────────────────┼────────────┼───────────┤[/{PALETTE.table_header}]")
        self.log_console.print(f"│ [{PALETTE.warning}]Total filtered out[/{PALETTE.warning}]           │ [{PALETTE.warning}]{total_removed:>10,}[/{PALETTE.warning}] │ [{PALETTE.warning}]{(total_removed/total*100) if total > 0 else 0:>8.2f}%[/{PALETTE.warning}] │")
        self.log_console.print(f"│ [{PALETTE.success}]Retained for scoring[/{PALETTE.success}]         │ [{PALETTE.success}]{kept:>10,}[/{PALETTE.success}] │ [{PALETTE.success}]{(kept/total*100) if total > 0 else 0:>8.2f}%[/{PALETTE.success}] │")
        self.log_console.print(f"[{PALETTE.table_header}]└──────────────────────────────┴────────────┴───────────┘[/{PALETTE.table_header}]")
        self.log_console.print("")
    ...
    def print_classification_results(
        ...
        # Log: ASCII table with color
        self.log_console.print("")
        self.log_console.print(f"[bold {PALETTE.highlight}]Classification Results (threshold: {threshold}%):[/bold {PALETTE.highlight}]")
        self.log_console.print(f"[{PALETTE.table_header}]┌──────────────────────┬───────────┬────────────┐[/{PALETTE.table_header}]")
        self.log_console.print(f"[{PALETTE.table_header}]│ Type                 │ Count     │ Percentage │[/{PALETTE.table_header}]")
        self.log_console.print(f"[{PALETTE.table_header}]├──────────────────────┼───────────┼────────────┤[/{PALETTE.table_header}]")
        self.log_console.print(f"│ [{PALETTE.u12_highlight}]U12-type (total)[/{PALETTE.u12_highlight}]     │ [{PALETTE.u12_highlight}]{u12_count:>9,}[/{PALETTE.u12_highlight}] │ [{PALETTE.u12_highlight}]{u12_pct:>9.2f}%[/{PALETTE.u12_highlight}] │")
        self.log_console.print(f"│ [{PALETTE.u12_highlight}]U12-type (AT-AC)[/{PALETTE.u12_highlight}]     │ [{PALETTE.u12_highlight}]{atac_count:>9,}[/{PALETTE.u12_highlight}] │ [{PALETTE.u12_highlight}]{atac_pct:>9.2f}%[/{PALETTE.u12_highlight}] │")
        self.log_console.print(f"│ U2-type              │ {u2_count:>9,} │ {u2_pct:>9.2f}% │")
        self.log_console.print(f"[{PALETTE.table_header}]├──────────────────────┼───────────┼────────────┤[/{PALETTE.table_header}]")
        self.log_console.print(f"│ [bold]Total[/bold]                │ [bold]{total:>9,}[/bold] │ [bold]{100.0:>9.2f}%[/bold] │")
        self.log_console.print(f"[{PALETTE.table_header}]└──────────────────────┴───────────┴────────────┘[/{PALETTE.table_header}]")
        self.log_console.print("")
    ...
    def print_dinucleotide_boundaries(
        ...
        # Log: ASCII table with color
        self.log_console.print("")
        self.log_console.print(f"[bold {PALETTE.highlight}]Top {top_n} Splice Site Boundaries ({intron_type} introns):[/bold {PALETTE.highlight}]")
        self.log_console.print(f"[{PALETTE.table_header}]┌──────┬──────────────┬──────────┬───────────┐[/{PALETTE.table_header}]")
        self.log_console.print(f"[{PALETTE.table_header}]│ Rank │ Dinucleotide │ Count    │ Percent   │[/{PALETTE.table_header}]")
        self.log_console.print(f"[{PALETTE.table_header}]├──────┼──────────────┼──────────┼───────────┤[/{PALETTE.table_header}]")

        for i, (dnts, count) in enumerate(boundaries_to_show, 1):
            pct = (count / total * 100) if total > 0 else 0
            self.log_console.print(f"│ {i:>4} │ {dnts:>12} │ {count:>8,} │ {pct:>8.2f}% │")
    ...
    def print_file_tree(self, output_files: dict[str, str]):
        ...
        self.log_console.print("")
        self.log_console.print(f"[bold {PALETTE.highlight}]Output Files:[/bold {PALETTE.highlight}]")
        ...
        for i, (file_type, filepath) in enumerate(items):
            ...
            self.log_console.print(f"  {prefix} [{PALETTE.success}]{file_type}:[/{PALETTE.success}] [{PALETTE.path}]{filepath}[/{PALETTE.path}]")
    ...
    def print_training_config(self, species: str, threshold: float, seed: int, max_iter: int,
                            u12_count: int, u2_count: int, n_models: int, eval_mode: str,
                            fixed_c: float = None, n_optimization_rounds: int = None):
        """Print training configuration summary to log only."""
        # Log: Training configuration section
        self.log_console.print("")
        self.log_console.print(f"[bold {PALETTE.header}]Training Configuration:[/bold {PALETTE.header}]")
        ...
        self.log_console.print(f"  Reference data: {u12_count:,} U12-type, {u2_count:,} U2-type")
        ...
        if fixed_c:
            ...
            if n_optimization_rounds:
                self.log_console.print(f"  Optimization rounds: {n_optimization_rounds}")
        self.log_console.print("")

    def print_hyperparameter_results(self, optimized_c: float, cv_score: float,
                                    calibration_method: str, dual: bool, intercept_scaling: float):
        """Print hyperparameter optimization results to log only."""
        self.log_console.print("")
        self.log_console.print(f"[bold {PALETTE.header}]Hyperparameter Optimization:[/bold {PALETTE.header}]")
    ...
    def print_nested_cv_results(self, n_folds: int, mean_f1: float, std_f1: float,
                               mean_pr_auc: float, std_pr_auc: float, fold_results: list):
        ...
        """
        self.log_console.print("")
        self.log_console.print(f"[bold {PALETTE.header}]Nested Cross-Validation Results ({n_folds} folds):[/bold {PALETTE.header}]")
        ...
        self.log_console.print(f"  │ Fold(s)       │ F1 Score │ PR-AUC │ Train U12 │ Train U2 │ Test U12 │ Test U2 │")
        self.log_console.print(f"  ├───────────────┼──────────┼────────┼───────────┼──────────┼──────────┼─────────┤")
    ...
    def print_split_eval_results(self, n_u12_train: int, n_u2_train: int,
                                n_u12_test: int, n_u2_test: int,
                                test_f1: float, test_pr_auc: float,
                                n_u12_val: int = None, n_u2_val: int = None):
        """Print train/test split evaluation results to log only."""
        self.log_console.print("")
        self.log_console.print(f"[bold {PALETTE.header}]Train/Test Split Evaluation:[/bold {PALETTE.header}]")
        self.log_console.print(f"  Training set: {n_u12_train + n_u2_train:,} introns "
                             f"({n_u2_train:,} U2, {n_u12_train:,} U12)")
        if n_u12_val is not None and n_u2_val is not None:
            self.log_console.print(f"  Validation set: {n_u12_val + n_u2_val:,} introns "
                                 f"({n_u2_val:,} U2, {n_u12_val:,} U12)")
        self.log_console.print(f"  Test set: {n_u12_test + n_u2_test:,} introns "
    ...
    def print_ensemble_summary(self, n_models: int, models: list):
        ...
        """
        self.log_console.print("")
        self.log_console.print(f"[bold {PALETTE.header}]Trained Ensemble ({n_models} models):[/bold {PALETTE.header}]")

        for i, model in enumerate(models, 1):
            self.log_console.print(f"  Model {i}/{n_models}:")
            self.log_console.print(f"    Training samples: {model.train_size:,} ({model.u12_count:,} U12, {model.u2_count:,} U2)")
